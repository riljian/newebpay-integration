...
const NormalPayForm: FC<{ mpgGateway: string }> = ({ mpgGateway }) => {
  ...
  return (
    <Formik
      ...
      onSubmit={(values) => {
        axios
          .post('/api/v1/orders', transformValuesToPayload(values as Values))
          .then(({ data }) => {
            ...
            <Stack spacing={1}>
              <Box>
                ...
                <FormikCheckboxField
                  ...
                  label="是否支援 WebATM"
                />
                <FormikCheckboxField name="enableVacc" label="是否支援轉帳" />
                <FormikCheckboxField
